...
export default function PortfolioDetailModal({
  ...
  return (
    <AnimatePresence>
      <motion.div onClick={closeModal} className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50 p-4">
        <motion.div
        layoutId={`portfolio-card-${portfolio.id}`}
        className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          {/* Header with Close Button */}
          <div className="sticky top-0 bg-white border-b border-slate-200 flex items-center justify-between p-4">
            <motion.h2 className="text-xl font-bold text-slate-900">
              {portfolio.name}
            </motion.h2>
            <button
              onClick={closeModal}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              {/* Main Image */}
              <motion.div layoutId={`portfolio-image-${portfolio.id}`} className="rounded-lg overflow-hidden h-80 bg-slate-200">
                <img
